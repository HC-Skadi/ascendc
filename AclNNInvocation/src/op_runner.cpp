...
bool OpRunner::RunOp()
{
    ...
    //int64_t padding, stride, dilationOptional, datalayout;
    auto ret =
        aclnnPreluGetWorkspaceSize(inputTensor_[0], inputTensor_[1], outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Get Operator Workspace failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
    INFO_LOG("Execute aclnnPreluGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnPrelu(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
    INFO_LOG("Execute aclnnPrelu success");
