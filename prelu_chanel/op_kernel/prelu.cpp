...
__global__ __aicore__ void prelu(GM_ADDR x, GM_ADDR weight, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    } else if constexpr (schMode == PRELU_TPL_CHANNEL_NCL_CONTIGUOUS_MODE) {
        op.InitChannelNclContiguous(x, weight, y, &tilingData, &pipe);
        op.ProcessChannelNclContiguous();
    ...
    } else if constexpr (schMode == PRELU_TPL_CHANNEL_NC_WEIGHT_REUSE_BY_INNER_MODE) {
        op.InitChannelNcWeightReuse(x, weight, y, &tilingData, &pipe);
        op.ProcessChannelNcWeightReuseByInner();
    ...
    } else if constexpr (schMode == PRELU_TPL_CHANNEL_NC_SPLIT_C_WEIGHT_REUSE_BY_INNER_MODE) {
        op.InitChannelNcSplitCWeightReuse(x, weight, y, &tilingData, &pipe);
        op.ProcessChannelNcSplitCWeightReuseByInner();
