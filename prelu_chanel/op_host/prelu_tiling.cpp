...
constexpr uint64_t NCL_CONTIGUOUS_MAX_INNER_SIZE = 128U;
constexpr uint64_t NCL_CONTIGUOUS_MIN_CHANNEL_SIZE = 64U;
...
static ge::graphStatus CalcTiling(
    ...
    PreluTilingData* tiling, uint32_t& usedCoreNum, bool& useNclContiguous, bool& useSplitLParallel,
    bool& useNcWeightReuse,
    bool& useNcWeightReuseByInner, bool& useNcSplitCWeightReuse, bool& useNcSplitCWeightReuseByInner)
{
    useNclContiguous = false;
    ...
    uint64_t nclUsedCoreNum = ChooseCoreNumByBatch(coreLimit, batchSize, batchSize);
    uint64_t nclMinUsedCoreNum = std::min(coreLimit, NCL_CONTIGUOUS_MIN_CORE_NUM);
    if (innerSize > 1 && static_cast<uint64_t>(innerSize) < NCL_CONTIGUOUS_MAX_INNER_SIZE &&
        static_cast<uint64_t>(channelSize) >= NCL_CONTIGUOUS_MIN_CHANNEL_SIZE &&
        nclUsedCoreNum >= nclMinUsedCoreNum && nclWeightCacheSizeValid && nclWeightCacheBytes < usableUbSize) {
        ...
        if (nclRowElements <= nclMaxTileElements) {
            uint64_t rowsPerTile = nclMaxTileElements / nclRowElements;
            while (rowsPerTile > 0 &&
                   AlignUp(rowsPerTile * nclRowElements, blockElementNum) > nclMaxTileElements) {
                --rowsPerTile;
            }
            if (rowsPerTile >= 1) {
                uint64_t tileElements = AlignUp(rowsPerTile * nclRowElements, blockElementNum);
                uint64_t finalCoreNum = nclUsedCoreNum;
                tiling->tileLength = static_cast<int64_t>(tileElements);
                tiling->innerSizeAligned = static_cast<int64_t>(innerSizeAligned);
                tiling->usedCoreNum = static_cast<int64_t>(finalCoreNum);
    ...
    if (preferWeightReuseByRow && channelSize > 1 && batchSize > 0) {
        ...
        OP_CHECK_IF(
            ...
            OP_LOGE(context, "Prelu: C*alignedL exceeds uint64 range"),
            return ge::GRAPH_FAILED);
        uint64_t alignedRowElements = innerSize == 1 ?
            AlignUp(rowElements, blockElementNum) :
            static_cast<uint64_t>(channelSize) * innerSizeAligned;
        ...
        if (weightCacheSizeValid && weightCacheBytes < usableUbSize) {
            uint64_t ncBufferBytesPerElement = innerSize == 1 ?
                GetNcWeightReuseBufferBytesPerElement(dataType) : GetBufferBytesPerElement(dataType);
            uint64_t ncMaxTileElements =
                ((usableUbSize - weightCacheBytes) / ncBufferBytesPerElement /
                 alignedRowElements) *
                alignedRowElements;
            if (ncMaxTileElements >= alignedRowElements) {
                uint64_t finalCoreNum = ChooseCoreNumByBatch(coreLimit, batchSize, batchSize);
        ...
        if (innerSize == 1) {
            ...
            if (splitCTileLength >= blockElementNum) {
                ...
                OP_CHECK_IF(
                    ...
                    return ge::GRAPH_FAILED);
                uint64_t finalCoreNum = ChooseCoreNumByBatch(coreLimit, batchSize, totalTaskNum);
...
static ge::graphStatus PreluTilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(
        CalcTiling(context, ubSize, coreNum, totalNum, dataType, typeLength, weightMode, channelSize, innerSize,
            rowNum, tiling, usedCoreNum, useNclContiguous, useSplitLParallel, useNcWeightReuse, useNcWeightReuseByInner,
    ...
    if (weightMode == 1) {
        if (useNclContiguous) {
            tilingKey = GET_TPL_TILING_KEY(PRELU_TPL_CHANNEL_NCL_CONTIGUOUS_MODE);
