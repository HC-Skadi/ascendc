...
TEST_F(PreluKernelTest, test_channel_small_l_multi_row_run)
{
    constexpr size_t n = 1;
    constexpr size_t c = 20;
    constexpr size_t l = 3;
    constexpr size_t groupRows = 8;
    constexpr size_t innerSizeAligned = 8;
    constexpr size_t groupElements = groupRows * innerSizeAligned;
    constexpr size_t size = n * c * l;
    constexpr size_t tilingDataSize = sizeof(PreluTilingData);
    constexpr uint32_t numBlocks = 3;
    ...
    std::vector<float> xHost(size);
    for (size_t i = 0; i < size; ++i) {
        xHost[i] = static_cast<float>(static_cast<int>(i % 19) - 9);
    }
    std::vector<float> weightHost(c);
    for (size_t i = 0; i < c; ++i) {
        weightHost[i] = 0.02f * static_cast<float>(i + 1);
    }
    std::vector<float> yHost(size, 0.0f);
    ...
    tilingData->formerLength = 0;
    tilingData->tailLength = 0;
    tilingData->tileLength = groupElements;
    tilingData->channelSize = c;
    tilingData->innerSize = l;
    tilingData->innerSizeAligned = innerSizeAligned;
    tilingData->groupRows = groupRows;
    ...
    ICPU_RUN_KF((prelu<PRELU_TPL_CHANNEL_SMALL_L_MULTI_ROW_MODE>), numBlocks, x, weight, y, workspace, tiling);
